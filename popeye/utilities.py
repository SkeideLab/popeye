...
def regularizing_error_function(
    parameter, bundle, p_bounds, thr=0.10
):  # pragma: no cover
    # if out of bounds
...
def regularizer(bundle, p_grid, p_bounds, Ns=None):  # pragma: no cover
    p0 = brute(
        regularizing_error_function,
        (p_grid,),
        args=(bundle, p_bounds),
        Ns=Ns,
        finish=None,
        full_output=True,
    )
...
def spm_hrf(delay, tr, oversampling=1, time_length=32.0, onset=0.0):
...
def glover_hrf(delay, tr, oversampling=1, time_length=32.0, onset=0.0):
...
def recast_xval_results(
    output, bootstraps, indices, grid_parent, overloaded=False, ncpus=30
):
    ...
    def parallel_loader(index):  # pragma: no cover
        ...
        # assign
        estimates[index[0], index[1], index[2]] = np.concatenate(
            (params, cod[:, np.newaxis], rsq[:, np.newaxis]), -1
        ).T
...
def gradient_descent_search(
    data, error_function, objective_function, parameters, bounds, verbose
):
...
def brute_force_search(
    data, error_function, objective_function, grids, Ns=None, verbose=False
):
...
def double_gamma_hrf(delay, tr, fptr=1.0, integrator=trapz):
    ...
    hrf = (
        (t ** (alpha_1) * beta_1**alpha_1 * np.exp(-beta_1 * t)) / gamma(alpha_1)
    ) - c * (
        ...
        for bootstrap in xrange(bootstraps):
            for voxel in xrange(data.shape[0]):
                ...
                resample_idx = np.random.choice(
                    np.arange(data.shape[1] - 1), resample, replace=False
                )
    ...
    for bootstrap in xrange(bootstraps):
        for voxel in xrange(data.shape[0]):
            ...
            if kfolds == 1:  # leave one out
                ...
                trn_idx = np.random.choice(
                    runs, np.int(len(runs) / kfolds), replace=False
                )
...
def multiprocess_bundle(
    Fit, model, data, grids, bounds, indices, auto_fit=True, verbose=1, Ns=None
):
    # num voxels
...
def parallel_xval(args):
    ...
    # fit the data
    fit = Fit(model, trn_data, grids, bounds, Ns, voxel_index, auto_fit, verbose)
...
def parallel_bootstrap(args):
    ...
    # fit the data
    fit = Fit(model, data, grids, bounds, Ns, voxel_index, auto_fit, verbose)
...
def hrf_derivative_dispersion(delay, tr, fptr=1.0, integrator=trapz):
    r"""The double gamma hemodynamic reponse function (HRF).
    The user specifies only the delay of the peak and undershoot.
    The delay shifts the peak and undershoot by a variable number of
    seconds. The other parameters are hardcoded. The HRF delay is
    ...
    hrf = np.stack(
        [spm_hrf(tr, 1), spm_time_derivative(tr, 1), spm_dispersion_derivative(tr, 1)],
        axis=1,
    )

    if integrator:  # pragma: no cover
        hrf /= integrator(hrf, axis=0)
